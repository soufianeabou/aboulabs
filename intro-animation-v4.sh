...
export default function PageIntro() {
  ...
  return (
    <>
      <style>{`
        @keyframes glowPulse {
          0%,100% { filter: drop-shadow(0 0 20px rgba(0,245,196,0.55)) drop-shadow(0 0 6px rgba(57,255,154,0.35)); }
          50%      { filter: drop-shadow(0 0 60px rgba(0,245,196,1)) drop-shadow(0 0 24px rgba(57,255,154,0.9)) drop-shadow(0 0 100px rgba(0,229,255,0.45)); }
        }
        @keyframes floatY {
        ...
        @keyframes letterSpacingIn {
          ...
          to   { letter-spacing:0.3em; opacity:0.7; }
        }
      `}</style>
      ...
      <div style={{
        ...
        {gte('scanline', phase) && !gte('wordmark', phase) && (
          <div style={{
            ...
            background: 'linear-gradient(90deg, transparent, rgba(0,245,196,0.55) 50%, transparent)',
        ...
        {/* ── Corner accents — 56px ── */}
        {[
          { top:'10%',    left:'5%',   borderTop:'1px solid rgba(0,245,196,0.55)',    borderLeft:'1px solid rgba(0,245,196,0.55)',    delay:'0s'    },
          { top:'10%',    right:'5%',  borderTop:'1px solid rgba(0,245,196,0.55)',    borderRight:'1px solid rgba(0,245,196,0.55)',   delay:'0.15s' },
          { bottom:'10%', left:'5%',   borderBottom:'1px solid rgba(0,245,196,0.55)', borderLeft:'1px solid rgba(0,245,196,0.55)',    delay:'0.3s'  },
          { bottom:'10%', right:'5%',  borderBottom:'1px solid rgba(0,245,196,0.55)', borderRight:'1px solid rgba(0,245,196,0.55)',  delay:'0.45s' },
        ].map((s, i) => (
          <div key={i} style={{
            position: 'absolute', width: '56px', height: '56px',
            opacity: show('frame') ? 1 : 0,
            transition: `opacity 0.5s ${s.delay}`,
            animation: show('mark-glow') ? `blink 2.5s ease-in-out infinite ${s.delay}` : 'none',
            top: s.top, left: (s as any).left, right: (s as any).right,
            bottom: (s as any).bottom,
            borderTop: (s as any).borderTop,
            borderLeft: (s as any).borderLeft,
            borderRight: (s as any).borderRight,
            borderBottom: (s as any).borderBottom,
          }} />
        ))}
        ...
        <div style={{
          position: 'absolute', top: 'calc(50% - 1px)', right: '50%',
          height: '1px',
          background: 'linear-gradient(270deg, #00f5c4, transparent)',
          animation: gte('wordmark', phase) ? 'slideLineL 0.6s cubic-bezier(0.23,1,0.32,1) forwards' : 'none',
          width: 0,
        }} />
        <div style={{
          ...
          animation: gte('wordmark', phase) ? 'slideLineR 0.6s cubic-bezier(0.23,1,0.32,1) forwards' : 'none',
          width: 0,
        }} />
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '3px', marginBottom: '6px' }}>
            {[10,18,8,28,14,24,9,32,12,22,7,18,26,11,20].map((h, i) => (
              <div key={i} style={{
                width: '2px', height: `${h}px`,
                background: `rgba(0,245,196,${0.25 + (i % 3) * 0.18})`,
                animation: `barWave ${0.35 + i * 0.06}s ease-in-out infinite ${i * 0.04}s`,
              }} />
            ))}
          </div>
          {['SYS_INIT', 'BUILD // 2025', 'MOROCCO'].map((line, i) => (
            <div key={i} style={{
              fontFamily: 'var(--font-jetbrains)',
              fontSize: '0.6rem', letterSpacing: '0.18em',
              color: i === 2 ? 'rgba(0,245,196,0.65)' : 'rgba(0,245,196,0.38)',
            }}>
              {line}
            </div>
          ))}
        ...
        <div style={{
          ...
          {['LOADING', 'ASSETS', 'ABOULABS'].map((line, i) => (
            <div key={i} style={{
              fontFamily: 'var(--font-jetbrains)',
              fontSize: '0.6rem', letterSpacing: '0.18em',
              color: i === 2 ? 'rgba(0,245,196,0.65)' : 'rgba(0,245,196,0.38)',
        ...
        {show('mark-glow') && [
          ...
          { left:'53%', size:2, delay:'1.6s', dur:'1.9s' },
        ].map((p, i) => (
          <div key={i} style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            transform: show('wordmark') ? 'translateY(0)' : 'translateY(22px)',
            transition: 'opacity 0.6s ease, transform 0.6s cubic-bezier(0.23,1,0.32,1)',
            filter: 'drop-shadow(0 0 14px rgba(0,245,196,0.28))',
            marginBottom: '18px',
          }}>
          ...
          <div style={{
            ...
            animation: show('details') ? 'letterSpacingIn 0.9s ease forwards' : 'none',
            marginBottom: '14px',
          }}>
          ...
          <div style={{
            ...
            opacity: show('details') ? 1 : 0,
            animation: show('details') ? 'fadeUp 0.6s ease 0.25s forwards' : 'none',
          }}>
            <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: '#00f5c4', boxShadow: '0 0 8px #00f5c4', display: 'inline-block' }} />
            <span style={{ fontFamily: 'var(--font-jetbrains)', fontSize: '0.58rem', letterSpacing: '0.25em', color: 'rgba(0,245,196,0.38)' }}>
              MOROCCO · 2025
            </span>
            <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: '#39ff9a', boxShadow: '0 0 8px #39ff9a', display: 'inline-block' }} />
          </div>
        </div>

        {/* Progress bar */}
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '2px', background: 'rgba(0,245,196,0.08)' }}>
          <div style={{
            height: '100%',
...
ENDOFFILE
echo "✅ Done — Morocco only, bigger logos (mark 140px, wordmark 380px)"
